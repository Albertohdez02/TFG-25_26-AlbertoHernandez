...
SOL_ACO = ROOT / "solutions_aco_v3"        # ACO + ALNS Etapa 3
SOL_ACO_V2 = ROOT / "solutions_aco"        # ACO post-Fases A+B+C+D (v2)
SOL_RND = ROOT / "solutions_random"
OUT_DIR = ROOT / "graficas_v3"
...
ax.set_title("Gap respecto a best — ACO Etapa 3 (ALNS+SA) vs Random (600s/inst)")
ax.legend(); ax.grid(axis="y", alpha=0.3)
fig.tight_layout()
...
ax.set_title("ACO+ALNS (Etapa 3) vs Random — puntos bajo y=x significan que ACO es mejor")
...
ax.set_ylabel("Gap respecto a best (%)")
ax.set_title("Gap a best i01–i30 — ACO+ALNS (Etapa 3) vs Random (600s/inst)")
...
ax.set_title("Coste agregado por componente — ACO+ALNS (Etapa 3) vs Random vs best (600s/inst)")
ax.legend(); ax.grid(axis="y", alpha=0.3)
fig.tight_layout()
